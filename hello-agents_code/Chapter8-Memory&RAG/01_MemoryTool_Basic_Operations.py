...
语义记忆存到Qdrant向量数据库和Neo4j图数据库中
情景记忆存到Qdrant向量数据库中
感知记忆按照模态（text/image/audio）存储到不同的向量数据库中
工作记忆存到本地
"""
...
import logging
# 抑制 qdrant_store 的 Pydantic 验证错误日志（qdrant-client 1.7.3 与云端服务版本不兼容导致，不影响功能）
logging.getLogger("hello_agents.memory.storage.qdrant_store").setLevel(logging.CRITICAL)
...
def memory_tool_execute_demo():
    ...
    memory_tool = MemoryTool(
        user_id="demo_user",
        # 声明四种记忆类型，模拟人类认知系统的四个层面：[工作记忆, 情景记忆, 语义记忆, 感知记忆]
